...
public class PageDtoToInlineMarkupConverter {
    ...
    public InlineKeyboardMarkup convert(PageDtos<CurrencyDto> source,
                                        Currency currency,
                                        Currency.Operation operation, String city, String size) {
        ...
        if (!source.isFirst()) {
            buttons.add(InlineKeyboardButton.builder()
                    .text("back")
                    .callbackData(callBackData + " " + (source.getPage() - 1) + " " + city + " " + size)
                    .build()
            );
        }
